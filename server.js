...
app.get('/product', function (req, res) {
  db.all('SELECT * FROM product WHERE obsoleted="non-obsoleted"', (err, rows) => {
    if (err) {
...
app.post('/cart', function (req, res) {
  ...
  db.run(`INSERT INTO cart_item (product_id) VALUES (${req.query.productId})`, (err) => {
    ...
    db.all('CASE WHEN p.typr="B" and p.price > 20 THEN UPDATE p.obsoleted="obsoleted" ELSE p.obsoleted="non-obsoleted"', (err, rows) => {
      res.status(201).json({ "data": rows });
    })
